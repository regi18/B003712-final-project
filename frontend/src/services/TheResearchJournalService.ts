...
export default class TheResearchJournalService {
   ...
   */
  static async getAll(): Promise<ResearchJournalArticle[]> {
  ...
  /**
   * TODO make the api return the list alreadyt summarized and than remove getAll()
   * 
   * Same as getAll(), but only returns the first 30 words of each article.
   * @returns A promise that resolves to an array of ResearchJournalArticle objects.
   */
  static async getAllSummary(): Promise<ResearchJournalArticle[]> {
    const MAX_WORDS = 50;

    return TheResearchJournalService.getAll().then((articles) => {
